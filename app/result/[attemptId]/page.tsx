    ...
    <main className="max-w-2xl mx-auto px-4 py-10">
      <div className="card p-7 flex items-center gap-6 flex-wrap">
        ...
        <div>
          ...
          <div className="text-sm text-[#5C6B7E]">Tiempo utilizado: <b className="font-mono text-ink">{fmtClock(durationSec)}</b></div>
          <div className="text-sm text-[#5C6B7E]">{exam?.title}</div>
        </div>
      </div>

      <div className="card p-6 mt-4">
        <h3 className="font-disp text-base text-ink mb-1">Tu desempeño por tema</h3>
        <p className="text-sm text-[#5C6B7E] mb-3">Dónde estuviste más sólido y dónde conviene reforzar.</p>
        <div>
          {topics.map(([t, v]) => {
            ...
            return (
              <div key={t} className="flex items-center gap-3 my-2">
                ...
                <div className="font-mono text-xs text-[#5C6B7E] w-14 text-right">{v.ok}/{v.tot}</div>
              </div>
            );
          })}
        </div>
      ...
      {review.length > 0 && (
        <div className="card p-6 mt-4">
          ...
          <div className="flex flex-col gap-2">
            {review.map((r) => (
              <div key={r.number} className="flex items-center gap-3 text-[13.5px] p-2.5 border border-[#EAF0F8] rounded-lg">
                <b className="font-mono w-9">{String(r.number).padStart(2, "0")}</b>
                <span className="flex-1 text-[#5C6B7E]">{r.topic}</span>
